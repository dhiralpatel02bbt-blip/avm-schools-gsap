...
// ============================================================
// SCHOOL INFO SECTION - pinned image + staged title/circle reveal
// ============================================================
(function initSchoolInfoOverlap() {
  const imgSec = document.querySelector(".bbt-FA-img-sec");
  const newsSection = document.querySelector(".news-section");
  if (!imgSec || !newsSection) return;
  ...
  const movingEls = [imgText, circle, mainTitle].filter(Boolean);

  const mobileCircleX = () => (window.innerWidth < 577 ? -93 : 0);
  const enterY = () => (window.innerWidth < 768 ? 54 : 86);
  const exitY = () =>
    -window.innerHeight * (window.innerWidth < 768 ? 0.74 : 0.88);
  const newsLiftY = () =>
    window.innerHeight * (window.innerWidth < 768 ? 1.02 : 1.32);

  gsap.set(imgSec, { zIndex: 1, overflow: "hidden" });
  gsap.set(newsSection, {
    position: "relative",
    zIndex: 5,
    y: newsLiftY,
    force3D: true,
  ...
  const schoolInfoTL = gsap.timeline({
    ...
    scrollTrigger: {
      ...
      start: "top top",
        ...
        "+=" +
        Math.round(
          window.innerHeight * (window.innerWidth < 768 ? 1.92 : 2.12),
        ),
      scrub: 1.15,
      pin: true,
      pinSpacing: false,
  ...
  schoolInfoTL
    .to(imgText, { autoAlpha: 1, y: 0, duration: 0.22 }, 0)
    .to(
      circle,
      {
        autoAlpha: 1,
    ...
    .to(mainTitle, { autoAlpha: 1, y: 0, duration: 0.24 }, 0.12)
    .to({}, { duration: 0.22 })
    .to(
      movingEls,
      {
...
(function initImgSecAnimation() {
  ...
  const circleSec = document.querySelector(".bbt-FA-circle-sec");
  ScrollTrigger.create({
    trigger: circleSec || imgSec,
    start: circleSec ? "bottom top" : "top top",
    once: true,
