...
(function initBubbleSectionV2() {
  ...
  function buildMobileBubbleTimeline() {
    ...
    bubbleCircles.forEach((circle, index) => {
      ...
      // When circle 1 becomes active, shrink circle 0 like other passed circles
      if (index === 1) {
        const c0 = bubbleCircles[0];
        const c0Heading = c0.querySelector("h2");
        const c0Body = c0.querySelector("p");
        bubbleTimeline.to(
          c0,
          {
            scale: 0.72,
        ...
        if (c0Body) {
          bubbleTimeline.to(c0Body, { autoAlpha: 0.3, duration: 0.2 }, at);
        }
      }
  ...
  function buildBubbleTimeline() {
    ...
    bubbleCircles.forEach((circle, index) => {
      ...
      // to match the same inactive treatment other circles get when passed
      if (index === 1) {
        const c0 = bubbleCircles[0];
        const c0Heading = c0.querySelector("h2");
        const c0Body = c0.querySelector("p");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (newsSection) {
    ...
    if (
      ...
      var existingPosition = window.getComputedStyle(newsSection).position;
      if (existingPosition === "static") {
        newsSection.style.position = "relative";
      }
      newsSection.appendChild(yellowOverlay);
      ...
      gsap.to(yellowOverlay, {
        ...
        scrollTrigger: {
          trigger: newsSection,
          start: "center top", // starts when middle of newsSection hits viewport bottom
          end: "bottom top", // fully yellow when bottom of newsSection hits viewport bottom
          scrub: 1.2,
          invalidateOnRefresh: true,
        },
      });
